...
- A path-transposition experiment that appears after any landmark route is completed. One action moves the whole route to the clockwise fifths neighbor, resets the learner to field one, and explicitly separates changed Do and physical frequencies from invariant field order, roles, root offsets, and internal interval shapes. Choosing another Do directly on the fifths compass applies the same safe reset, so stale progress cannot leak between centers.
