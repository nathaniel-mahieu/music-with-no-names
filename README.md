...
The current milestone is **Live Interval Assumption Bridge v1.55**. It provides:
...
- An Interval Echo source-versus-replay experiment that freezes a two-note spacing, groups every later attempt as a fresh non-overlapping pair, and draws both hand shapes on one keyboard coordinate. A matched replay exposes the equal-key span and frequency ratio that survived while separately reporting hand-center shift, direction, absolute fundamentals, hertz gap, and spectrum-dependent roughness and overlap. A mismatch preserves the frozen source and states the attempted span so the learner can repair one relationship without confusing register invariance with identical sound or experience.
