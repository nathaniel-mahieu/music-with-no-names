...
const FOCUS_LENSES: Array<{ id: FocusLens; label: string; description: string }> = [
  ...
  { id: "score-flow", label: "Score Flow · upload score", description: "Align a MusicXML score with your silent playing, hands, intervals, and repair loops." },
...
function isPersistedHudEventList(value: unknown): value is HudNoteEvent[] {
  ...
  return value.every((candidate) => {
    ...
    const event = candidate as Partial<HudNoteEvent>;
    if (!Number.isInteger(event.id) || event.id! < 1 || ids.has(event.id!)) return false;
    ids.add(event.id!);
    return Number.isInteger(event.note) && event.note! >= 0 && event.note! <= 127
      ...
      && (event.keyReleaseMs == null || Number.isFinite(event.keyReleaseMs))
      && (event.releaseMs == null || Number.isFinite(event.releaseMs))
      && (event.releaseReason == null || event.releaseReason === "key" || event.releaseReason === "pedal")
      && isMidiNoteList(event.fieldNotes);
  });
}
...
export function PianoLab() {
    ...
    const hydrationTask = window.setTimeout(() => {
      ...
      try {
        ...
        if (raw) {
          ...
          if ((saved.version === 2 || saved.version === 3 || saved.version === 4 || saved.version === 5 || saved.version === 6 || saved.version === 7 || saved.version === 8 || saved.version === 9 || saved.version === 10 || saved.version === 11 || saved.version === 12 || saved.version === 13 || saved.version === 14 || saved.version === 15 || saved.version === 16 || saved.version === 17 || saved.version === 18 || saved.version === 19 || saved.version === 20 || saved.version === 21 || saved.version === 22 || saved.version === 23 || saved.version === 24 || saved.version === 25) && isPersistedHudEventList(saved.phraseEvents)) {
  ...
  const strikeEchoScreenKey = (note: number) => {
    const atMs = currentHudTime();
    captureArmedDoAttack(note);
    addEvent(note, 104, 0, "screen", Array.from(new Set([...midi.notes.keys(), note])), atMs);
    releaseEvent(note, 0, "screen", atMs + 1, false);
  };

  const clearAll = () => {
    try { window.sessionStorage.removeItem(ECHO_KEY_LESSON_STORAGE_KEY); } catch { /* EchoKey still resets in memory when session storage is unavailable. */ }
    doCaptureArmedRef.current = false;
    setDoCaptureArmed(false);
    ...
    setEchoKeyResetVersion((current) => current + 1);
  ...
  return (
    <section className={`advanced-lab piano-lab piano-hud${focusLens === "score-flow" ? " is-score-flow" : ""}`} aria-labelledby="piano-hud-title">
      ...
      <nav className="piano-focus-lenses" aria-label="Learning focus">
        {FOCUS_LENSES.map((lens) => <button key={lens.id} type="button" className={lens.id === "echo-key" ? "is-ear-first" : undefined} aria-pressed={focusLens === lens.id} onClick={() => selectFocusLens(lens.id)}><strong>{lens.label}</strong><span>{lens.description}</span></button>)}
      </nav>
      ...
      {focusLens === "echo-key" ? <PianoEchoKeyHud
        key={echoKeyResetVersion}
        events={phraseEvents}
        activeNotes={activeNoteNumbers}
        doMidi={doMidi}
        scale={scale}
