...
| Notated pitch spelling | An authored letter, accidental, and octave whose staff location and conventional identity remain explicit; enharmonic spellings may share one piano key while carrying different written relationships. | MIDI key number, acoustic frequency, detected key, or arbitrary automatic respelling. |
| Generic interval | Written line/space distance counted inclusively, such as a second or third, independent of its exact semitone size. | Semitone distance: a written third may span three or four semitones. |
| Sight-reading shape | A bounded score relationship—interval, contour, group, chord silhouette, fixed point, transformation, or coordination pattern—used as an attention and retrieval scaffold. | A claim that notation has one visual meaning, a detected phrase, an observed hand shape, or proof of learning. |
| Pre-movement cue | An exercise-authored next landing or territory prompt shown before an attack so the learner can prepare a relationship. | Measured fingering, gaze, posture, physical tension, motor readiness, or ergonomic advice. |
