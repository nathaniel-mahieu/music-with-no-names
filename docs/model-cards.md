...
- **Version:** `mwno-piano-12` for the bounded sixty-second phrase, seven-attack microscope, duration/pedal ribbon, articulation lane, phrase-local motif transformations, focus lenses, phrase-stabilized scale frame, cyclic scale fingerprint, performed-evidence tonal gravity, silent resolution forks, correctable chord gestures, causal chord comparison, explicit voice-leading strands, silent chord targets, playable landmark paths, the listener-reported Personal Character Map, and selectable assumed sound models; spectral evidence reuses `mwno-sonority-1`.
- **Inputs:** up to sixty seconds of local note-on events (bounded to 256 attacks), velocity, key release, sounding release, pedal extension, simultaneous held/pedaled field, optional chord-boundary corrections, an optional locked movable Do, the current scale frame, a locally selected ghost target or generated landmark path, path-step progress, and optional conventional-label visibility. Coordinated detail views inspect the latest seven attacks.
...
- **MIDI behavior and privacy:** permission and messages stay in the browser. Note-on, note-off, velocity, and sustain drive only the live local visualization; Piano Lab never synthesizes, records, or routes audio. The bounded sixty-second phrase and local learning settings use tab-scoped session storage so they survive lab navigation and disappear when that tab session ends. Explicit Personal Character reports use local browser storage so they can reappear in the Personal Lens and join export or two-step deletion; nothing is uploaded.
