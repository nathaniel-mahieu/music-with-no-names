...
- **Implementation status:** Scale Lab v1.23 uses movable solfège as a culturally situated relational mnemonic, not a physical universal. It declares a 2:1 octave realized in twelve equal logarithmic slices, applies physically invariant small/medium/large gap labels, and keeps melodic distance separate from matched harmonic-spectrum interaction. Action-first copy asks learners to listen before opening measurements. Context A/Bs collect the learner’s report before interpretation. Scale-specific inner-hearing phrases require a degree choice and restore the answer in context. Piano Lab v1.25 then treats equal-tempered keys, conventional note names, scale names, and the circle of fifths as optional learning overlays on top of frequency and interval relationships. Its MIDI path is deliberately silent and analysis-only. Scale candidates are labeled as compatible frames rather than detected truth, and crunch, tonal pull, and arrival evidence remain separate model dimensions. The presets are teaching examples, not a claim that one tuning or scale system is natural or culturally neutral.
