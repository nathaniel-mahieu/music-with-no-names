...
- The Piano Lab uses native buttons for every silent on-screen key, attack, learning lens, guided-scale action, interval-echo action, ghost target, chord-boundary correction, inherited-membership correction, and grouped chord gesture; native selects for MIDI device and chord timing granularity; text-redundant held, sustained, duration, pedal-tail, inherited, excluded, guided-step, ghost, scale, chord-member, and voice-motion states; named SVG summaries for the sixty-second phrase ribbon, staff, frequency, fifths, interval, voice-leading, and evidence views; concise polite live explanations for both the HUD and guided walk; and line/diamond/dash/cross/check/arrow shapes as well as color for perceptual traces, voice strands, walk progress, inherited exclusions, ghost keys, and chord brackets.
