...
- The Piano Lab uses native buttons for every silent on-screen key, attack, learning lens, interval-echo action, chord-boundary correction, and grouped chord gesture; native selects for MIDI device and chord timing granularity; text-redundant held, sustained, duration, pedal-tail, inherited, scale, and chord-member states; named SVG summaries for the phrase ribbon, staff, frequency, fifths, interval, and evidence views; a concise polite live explanation; and line/diamond/dash shapes as well as color for perceptual traces and chord brackets.
