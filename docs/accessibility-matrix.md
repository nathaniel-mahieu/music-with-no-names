...
- The Piano Lab uses native buttons for every silent on-screen key, attack, learning lens, interval-echo action, ghost target, chord-boundary correction, and grouped chord gesture; native selects for MIDI device and chord timing granularity; text-redundant held, sustained, duration, pedal-tail, inherited, ghost, scale, chord-member, and voice-motion states; named SVG summaries for the sixty-second phrase ribbon, staff, frequency, fifths, interval, voice-leading, and evidence views; a concise polite live explanation; and line/diamond/dash shapes as well as color for perceptual traces, voice strands, ghost keys, and chord brackets.
