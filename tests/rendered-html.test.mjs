...
test("removes the disposable starter and keeps audio safety visible in source", async () => {
  ...
  assert.match(scaleLab, /Hear home\. Walk gaps\. Predict motion/);
  assert.match(scaleLab, /This is transposition/);
  assert.match(scaleLab, /retrieval before reveal/i);
  assert.match(scaleLab, /physical interaction ≠ contextual pull ≠ musical value/);
  ...
  assert.match(scaleLab, /Local geometry—not tonal pull/);
