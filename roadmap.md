...
- [x] **K2.6.97** Add Interval Glow HUD v2.11 as a dedicated Piano focus: unroll the full 88-key range into equal semitone cells; map every newest signed attack interval onto a labeled 1–12 color ring without discarding its exact compound distance; keep same-key repetition distinct from positive octave multiples; decompose a voiced field into low-to-high adjacent gaps and every unordered pair; place added-octave pairs inward with smaller marks; retain a bounded twenty-four-attack center timeline with pitch height, attack order, signed interval links, and timing-window chord halos; supply interval-specific contrast prompts rather than emotion or quality verdicts; reuse the shared silent MIDI connection, movable Do, selected route, grouping window, and on-screen keyboard; and verify arithmetic, persistence, accessible descriptions, forced colors, responsive layout, and production deployment.
- [x] **K2.6.98** Add Felt Interval Gestures v2.12: distinguish sequential intervals from close-time chord membership instead of treating MIDI packet order inside one group as isolated melody; turn between-moment motion into a one-shot directional arc whose bend, weight, and pace follow signed semitone span and onset gap; turn simultaneously interpreted all-pairs spacing into a woven bloom whose spread, edge, and core separately encode modeled openness, roughness, and fusion under the selected equal-level teaching spectrum; retain stable 1–12 color and exact compound-distance markings; offer embodied motion and texture words only as learner-testable prompts; add a direct broken-then-together ear experiment; and verify timing classification, exact-spacing texture classification, non-color distinctions, reduced motion, forced colors, responsive layout, and production deployment.
- [x] **K2.6.99** Add Sight Shapes v2.13 as a silent score-to-keyboard relationship focus: provide eighteen curated exercises across Understand (intervals, lines, chords, fixed points, and transformations), Embody (groups, pre-movement, expansion/contraction, and coordination), Integrate (indications, musical ideas, and two learner-supplied words), and Practice Mindset (diagnose one relationship, then retry); retain explicit notated pitch spelling and keep written generic interval from line/space count separate from signed semitone distance; compare learner input in exact-position and uniformly transposable-shape modes; accept the shared MIDI or silent browser-key event stream; interpret intended vertical groups against the selected 80, 160, or 320 ms chord window; progressively fade staff, semitone, contour, territory, landing, group, chord-gap, anchor, and reflection scaffolds; state that MIDI cannot supply audio, fingering, gaze, physical tension, intention, emotion, correctness, or musical quality; and verify deterministic model and source contracts, minimum 44-pixel controls, forced-color and reduced-motion behavior, 1280/390/320-pixel no-overflow layouts, passing tests, and the production build.
- [x] **K2.6.100** Repair and harden Sight Shapes notation in v2.14: choose a low-complexity contextual tonic spelling for each complete authored shape; retain a target's exact staff spelling in matched MIDI overlays; implement conventional within-measure accidental carry and explicit natural cancellation; derive solfège and nearby labels from the same spelling; name fixed specimens that intentionally leave the selected route; freeze Do and route per attempt until explicit adoption; make chord assignment and transferable-shape inference independent of MIDI packet order; share the Piano HUD's adjacent-gap plus maximum-span grouping rule; separate pitch success from rhythm advice; restore preview-then-veil memory practice; derive folded sonorities from their actual pitch material; render shared chord stems, slurs, breath marks, and independent two-hand movement lanes; disable staff-colliding registers; auto-follow future score and keyboard destinations on narrow screens; and exhaustively verify every authored spacing across twelve Do positions and three registers without double accidentals.
- [x] **K2.6.101** Ship the first EchoKey ear-first lesson in v2.15: add a prominent Piano focus that sequences explicit reference listening, silent audiation, learner-reported singing, transposition-tolerant keyboard finding, first-divergence diagnosis, bounded repair with assisted continuation, delayed duration-aware notation, an exact chromatic semitone strip, and calculated-plus-auditory transfer; snapshot and restore the tab-local exercise frame while rebasing any resumed Find boundary; reject pre-existing held/sustained tones, chord-like clusters, and extra attacks rather than truncate them; keep incoming MIDI silent; separate contour, exact directed semitone width, and supplied-anchor evidence; veil bounded, deletable, miss-only tab-scoped confusion summaries during blind recall; keep contextual spelling consistent and extreme-register notation visible; and verify deterministic scoring, audio cancellation safety, delayed disclosure, phase focus/announcements, momentary on-screen melodic attacks, keyboard navigation, responsive layout, and production routing.
- [x] **K2.6.102** Add Score Flow · Local MusicXML Coach v2.16: import MXL and score-partwise MusicXML entirely on-device; preserve authored spelling, voices, staves, rests, ties, durations, tuplets, meter and tempo changes, explicit fingerings, and marked arpeggiation without resolving external XML resources; turn simultaneous cross-staff attacks into target-aware practice landings; retain a thirty-minute/4,096-attack score take; align silent MIDI through short selected measure-and-hand loops; freeze each completed attempt; expose exact semitone and approximate white-key-width corrections, real keyboard territory, advisory fingering, chord gaps, first-divergence repair loops, and separate pitch, chord, roll, contour, interval, bass, pulse, release, and progress evidence; provide open, fade, and pitch-veiled ear-first layers plus bounded explicit reference audio; bundle only an original demo; validate the supplied unlicensed commercial score locally without redistributing it; and verify parser security, deterministic evaluation, responsive/non-color/accessibility contracts, full tests, production builds, and deployment.
- [x] **K2.6.103** Recompose Score Flow as an Immersive Reading Field v2.17: replace the repeated equal-card horizon and metric wall with one coordinated Form → Chunk → Landing hierarchy; render a bounded success/repair/waiting journey; generate deterministic two- through eight-landing reading groups from bar, rest, silence, staff, contour, and chord-anchor evidence while calling them suggested chunks rather than authored phrases; expose landmark, top-note semitone path, vertical gaps, rhythm cell, and retained notation-edge cues; rank nearby major, natural-minor, and pentatonic pitch collections while leading with the current measure's authored signature/mode, suppressing thin-evidence candidates, and refusing detected-key or goodness claims; keep the ear-first veil closed over note, interval, signature, range, chord, and collection evidence; teach the selected chord-arrival time as a visible gathering window; unify correct, missing, extra, and physical-correction glyphs on the focused landing; distinguish authored `score` fingerings from dashed `~` suggestions; rename the hand filter as upper/lower staff focus; move deep metrics and evidence limits into disclosures; compact the surrounding Piano shell so the playing surface stays above the fold; and verify chunk/collection models, local-key context, non-color states, accessible announcements, forced colors, responsive reflow, complete tests, production builds, and deployment.
- [x] **K2.6.104** Add Adaptive Chunk Practice v2.18: turn every suggested reading chunk into a stable, keyboard-accessible exact-attack practice target; prevent surrounding measure attacks from becoming misses; anchor leading, middle, and trailing extra attacks to the nearest written repair location; retain up to twelve frozen repetitions as separate pitch, chord-membership, semitone-spacing, arpeggiation, fixed-pulse, and key-up evidence; describe only repeated repair or consecutive alignment without inventing mastery or an aggregate score; recommend one active Landmark, Motion, Vertical, or Rhythm reading lens while keeping the others one click away; align the visible chord-gathering deadline with the evaluator's first-attack window; separate physically pressed keys from pedal-sustained tones at take completion; constrain local collection evidence to one authored key context; remove stale failed upload persistence; index large MusicXML imports in linear time; bound the measure navigator; and verify stable chunk identity, exact-range evaluation, repetition bounds, importer complexity, accessibility, type safety, full tests, production builds, and deployment.
- [x] **K2.6.105** Add Score Flow Live Response v2.19: make provisional score following active without an Arm step; give every raw MIDI or silent-key attack an immediate veil-safe receipt; hold an incomplete written chord on its current landing through the selected non-transitive gathering window; repeat completed study loops without mutating frozen reviews; expose disconnected, ready, received, paused, recording, and frozen states in one command bar; reconnect authorized MIDI devices without duplicate handlers or a lost first event; retain exact chunk selection and bounded repetition evidence; and verify raw-input response, capture boundaries, accessibility, type safety, browser performance, and deployment.
- [x] **K2.6.106** Add Ear–Eye–Hands Play Along v2.20: replace the cursor-only audition with a meter-aware audible and visible count-in; derive synthesized audio, a moving form/playhead cursor, current heard-note staff reveal, and MIDI timing evaluation from one bounded cue plan and performance clock; reject uninitialized or cross-origin browser timestamps; keep Easy exact semitone/millisecond feedback, Memory decay, and Hard pitch-veiled categorical feedback distinct; freeze natural and manually stopped synchronized takes with explicit misses; separate full-loop play-along from upper and bass previews; correct the reference gain/compressor order, use a bounded user loudness control, and retain safe equal-power mixing; throttle animation state to protect long-score browser sessions; and verify cue arithmetic, first-note timing, disclosure boundaries, live browser synchronization, accessibility, type safety, lint, production builds, and deployment.
- [x] **K2.6.107** Add the Veiled Listening Staff v2.21: keep a crisp grand-staff scaffold visible through idle, count-in, playback gaps, Stop, and review; display a browser-bounded seven-landing window with onset-proportional, collision-adjusted space, measure boundaries, clef orientation, pitch-explicit accidentals, collision-aware chord noteheads, ledger lines, and explicit octave displacement; omit future pitch positions from the rendered staff instead of blurring them; retain the clock-reached prefix in Easy, briefly reveal then re-cover it in Memory, and expose pulse, centered attack cardinality, and categorical direction in Hard until review; preserve the audible plan's start and end indexes so mid-loop voice previews, early stops, and truncated references cannot reveal unscheduled material; keep upper/bass disclosure identical to the synthesized voice; add non-color success/repair marks, reduced-motion concealment, forced-color notation, narrow-screen reflow, source contracts, and live browser disclosure checks.
- [x] **K2.6.108** Add the Ear-First Recording Bridge v2.22: separate `Listen · notes locked` from `Play + compare`; keep pitch, note count, staff height, target keys, and grading hidden throughout an ear-only pass and after Stop or completion; make `Reveal heard passage` an explicit post-listen action limited to the clock-reached prefix; accept browser-local audio recordings; align score landings to media times with multiple drift/rubato anchors and whole-map nudging; use the recording clock for score position and MIDI attack-timing evidence; retain MusicXML as the source of pitch and structure; and embed Spotify only as an official, independent listen-only player that never drives the cursor, reveal, or grading because Spotify prohibits synchronizing its recordings with visual media.
- [x] **K2.6.109** Add the One-Screen Playing Deck v2.23: fix stopped blind-listening passes so their clock-reached prefix remains available for learner-requested reveal; make reveal-state titles and source readiness honest; place score identity, source choice, practice disclosure, MIDI readiness, and review actions in a two-row desktop command deck; keep transport, listening staff, and hand correction together below it inside a 1280×720 playing viewport; move local-recording, Spotify, loop, cue, timing, volume, and togetherness configuration into bounded overlay drawers rather than allowing setup to displace the staff; preserve 44-pixel controls, focus visibility, screen-reader status, mobile stacking, and independent Spotify timing; and verify the supplied MXL, synthesized and local playback, explicit reveal, layout geometry, automated tests, lint, render, production, and GitHub Pages export.
...
- [x] Local Score Flow imports learner-supplied MXL/MusicXML, maps written attack moments into silent MIDI practice loops, and links first-divergence evidence to exact semitone, physical key-travel, advisory fingering, chord, interval, rhythm, duration, and ear-first repair views without uploading or redistributing the score.
- [x] Score Flow's immersive reading field now binds each focused landing to one suggested multi-landing chunk, a proportional rhythm path, a local-but-uncertain pitch-collection compass, a compact physical keyboard route, and one non-blended success/repair journey.
- [x] Score Flow's adaptive chunk cycle now lets the learner select one exact landing span, rehearse it without out-of-chunk misses, compare bounded per-dimension evidence across repetitions, and switch among Landmark, Motion, Vertical, and Rhythm reading lenses without treating one clean take as mastery.
- [x] Score Flow now responds in an always-on provisional live-follow state, gives every raw attack a veil-safe focal receipt before chord adjudication, repeats completed loops automatically, exposes connection/pause/review state in one command bar, reconnects previously authorized MIDI safely, and preserves an explicit frozen-review boundary for retained evidence.
- [x] Score Flow now establishes a shared audible start: its count-in, synthesized score, current-note staff reveal, form cursor, and silent MIDI timing comparison use one clock, while Easy, Memory, and Hard modes regulate how much pitch and timing information is disclosed.
- [x] Score Flow now keeps a persistent listening staff between ear and hands: future pitch positions remain covered, the audio-clock-reached prefix survives rests, Easy/Memory/Hard disclose progressively different amounts, and review reveals only scheduled onsets reached by that clock.
- [x] Score Flow now supports a fully hidden listen-first pass with learner-requested post-listen reveal, plus locally uploaded recordings whose score/media anchors drive the playhead and MIDI attack-timing comparison. Spotify remains an independent official reference embed and never enters the score clock, disclosure, or grading path.
